...
public class UserNewsCommentAdapter extends BaseListAdapter<UserNewsComment> {

    public UserNewsCommentAdapter(Context context, List<UserNewsComment> list) {
	super(context, list);
    }
	...
	if (null == convertView) {
	    convertView = LayoutInflater.from(mContext).inflate(R.layout.user_news_comment_item, null);
	}

	final UserNewsComment comment = this.list.get(position);
