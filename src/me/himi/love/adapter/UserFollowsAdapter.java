...
public class UserFollowsAdapter extends BaseListAdapter<FollowUser> {

    public UserFollowsAdapter(Context context, List<FollowUser> list) {
	super(context, list);
    }
	...
	if (null == convertView) {
	    convertView = LayoutInflater.from(mContext).inflate(R.layout.user_follow_item, null);
	}

	final FollowUser user = this.list.get(position);
	...
	tvVisitTime.setText(user.getAddTime());
