 ...
 */
public class EditArticleActivity extends BaseActivity implements OnClickListener {
	...
	case CAMERA_PHOTO:
	    ...
	    foruploadFiles.add(new File(facePhotoPath));
	    // 显示删除按钮
	    mTvDeleteImage.setVisibility(View.VISIBLE);
	    ImageView imageView = (ImageView) findViewById(R.id.iv_secret_image);
	    imageView.setVisibility(View.VISIBLE);
	    ImageLoader.getInstance().displayImage("file://" + facePhotoPath, imageView);
	    break;
	case GALLERY_PHOTO:
	    if (resultCode != RESULT_OK) {
		showToast("没有选择照片");
	    ...
	    //	    showToast(uri.);
	    String file = ActivityUtil.getRealFilePath(EditArticleActivity.this, uri);
	    ...
	    //	    // 显示删除按钮
	    mTvDeleteImage.setVisibility(View.VISIBLE);
	    imageView = (ImageView) findViewById(R.id.iv_secret_image);
	    imageView.setVisibility(View.VISIBLE);
